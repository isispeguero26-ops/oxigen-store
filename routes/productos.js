...
router.get('/agregar', requireLogin, (req, res) => {
    res.render('agregar');
});
...
router.post('/registro', async (req, res) => {
    ...
    db.query(
        ...
        [nombre, email, hash],
        (err) => {
            if (err) return res.render('registro', { layout: 'login-layout', error: 'El correo ya está registrado' });
            res.redirect('/login');
        }
    );
});
...
router.post('/pagar', requireLogin, (req, res) => {
    ...
    db.query('insert into ordenes (usuario_id, total) values (?, ?)', [usuarioId, total], (err, result) => {
        ...
        db.query('insert into orden_productos (orden_id, producto_id, nombre, precio, cantidad) values ?', [productos], (err) => {
            if (err) return res.status(500).send('error al guardar productos de la orden');
